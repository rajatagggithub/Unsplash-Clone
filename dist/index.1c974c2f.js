...
window.addEventListener("resize", function() {
    if (this.document.body.clientWidth < 738) scrollFraction = 8;
    else if (this.document.body.clientWidth < 484) scrollFraction = 11;
});
...
angleSmallLeft.addEventListener("click", function() {
    ...
    if (effectiveScrollValue <= 0) angleSmallLeft.style.visibility = "hidden";
});
angleSmallRight.addEventListener("click", function() {
    ...
    let effectiveScrollValue = categoriesRight.scrollLeft + nextScrollValue;
    if (effectiveScrollValue >= maxScrollValue) angleSmallRight.style.visibility = "hidden";
});
categoriesRight.addEventListener("scroll", ()=>{
    if (categoriesRight.scrollLeft == 0) angleSmallLeft.style.visibility = "hidden";
    else if (categoriesRight.scrollLeft > 0) angleSmallLeft.style.visibility = "visible";
    let maxScrollValue = categoriesRight.scrollWidth - categoriesRight.clientWidth;
    if (categoriesRight.scrollLeft < maxScrollValue) angleSmallRight.style.visibility = "visible";
    else if (categoriesRight.scrollLeft >= maxScrollValue) angleSmallRight.style.visibility = "hidden";
});
...
function changeSearchBoxPlaceholder() {
    if (searchBox.clientWidth < 254) searchBoxInput.placeholder = "Search images";
    else searchBoxInput.placeholder = "Search high-resolution images";
}
...
categoriesItems.forEach((item)=>{
    item.addEventListener("mouseout", function() {
        if (item.style.borderBottom != "2px solid black") item.style.color = "#767676";
    });
});
